//
//  LeafMultipartFormData.swift
//  Leaf
//
//  Created by Elias Abel on 27/4/17.
//
//
...
public class LeafMultipartFormData {
    ...
    struct BoundaryGenerator {
        ...
        static func randomBoundary() -> String {
            return String(format: "leaf.boundary.%08x%08x", arc4random(), arc4random())
        }
    ...
    private var bodyPartError: LeafError?
    ...
    public func writeEncodedData(to fileURL: URL) throws {
        ...
        if FileManager.default.fileExists(atPath: fileURL.path) {
            ...
            throw LeafError.parse(code: underlying._code, message: "Output stream file already exists.", object: fileURL, underlying: underlying)
        } else if !fileURL.isFileURL {
            let underlying = URLError(.badURL)
            throw LeafError.parse(code: underlying._code, message: "Output stream URL is invalid.", object: fileURL, underlying: underlying)
        }

        guard let outputStream = OutputStream(url: fileURL, append: false) else {
            let underlying = URLError(.cannotCreateFile)
            throw LeafError.parse(code: underlying._code, message: "Output stream creation failed.", object: fileURL, underlying: underlying)
        }
    ...
    private func encodeBodyStream(for bodyPart: BodyPart) throws -> Data {
        ...
        while inputStream.hasBytesAvailable {
            ...
            if let streamError = inputStream.streamError {
                throw LeafError.parse(code: streamError._code, message: "Input stream read failed.", object: inputStream, underlying: streamError)
            }
    ...
    private func write(_ buffer: inout [UInt8], to outputStream: OutputStream) throws {
        ...
        while bytesToWrite > 0, outputStream.hasSpaceAvailable {
            ...
            if let streamError = outputStream.streamError {
                throw LeafError.parse(code: streamError._code, message: "Output stream write failed.", object: outputStream, underlying: streamError)
            }
    ...
    private func setBodyPartError(_ reason: String, object: Any?, underlying: Error?) {
        ...
        bodyPartError = LeafError.parse(code: underlying?._code, message: reason, object: object, underlying: underlying)
    }
