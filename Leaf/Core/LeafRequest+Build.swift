...
public class LeafBuilder: Equatable {
    
    public static func == (lhs: LeafBuilder, rhs: LeafBuilder) -> Bool {
    ...
    public private(set) var contentLength: LeafRequest.LeafContentLength?
    
    public private(set) var accept: LeafContentType?
    
    public private(set) var acceptEncoding: [LeafRequest.LeafContentEncoding]?
    ...
    @discardableResult open func setContentLength(_ contentLength: LeafRequest.LeafContentLength?) -> Self {
    ...
    @discardableResult open func setAcceptEncodings(_ acceptEncodings: [LeafRequest.LeafContentEncoding]?) -> Self {
        self.acceptEncoding = acceptEncodings
        return self
    }
    
    @discardableResult open func addAcceptEncoding(_ acceptEncoding: LeafRequest.LeafContentEncoding?) -> Self {
    ...
    @discardableResult open func setBody(_ body: Data?) -> Self {
        if body != nil {
            ...
            if contentLength == nil, let length = body?.count {
                setContentLength(LeafRequest.LeafContentLength(length))
            }
        }
        self.body = body
        return self
...
extension LeafRequest {
    ...
    public func builder() -> LeafBuilder {
        return LeafRequest.builder(self)
    }

    public static func builder(_ leafRequest: LeafRequest) -> LeafBuilder {
        return LeafBuilder(leafRequest)
    }

    public static func builder(_ urlRequest: URLRequest) -> LeafBuilder? {
        return LeafBuilder(urlRequest)
    }

    public static func builder(_ url: URL) -> LeafBuilder {
        return LeafBuilder(url)
    }

    public static func builder(_ urlString: String) -> LeafBuilder? {
        return LeafBuilder(urlString)
    }

    public init(_ builder: LeafBuilder) {
        self.init(builder.url,
                  cache: builder.cache ?? .useProtocolCachePolicy,
                  timeout: builder.timeout ?? 60,
                  mainDocumentURL: builder.mainDocumentURL,
                  serviceType: builder.serviceType ?? .default,
                  contentType: builder.contentType,
                  contentLength: builder.contentLength,
                  accept: builder.accept,
                  acceptEncoding: builder.acceptEncoding,
                  cacheControl: builder.cacheControl,
                  allowsCellularAccess: builder.allowsCellularAccess ?? true,
                  method: builder.method ?? .GET,
                  headers: builder.headers,
                  body: builder.body,
                  bodyStream: builder.bodyStream,
                  handleCookies: builder.handleCookies ?? true,
                  usePipelining: builder.usePipelining ?? true,
                  authorization: builder.authorization ?? .none)
    }
}

extension LeafBuilder {
