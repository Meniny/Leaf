...
//  Leaf
//
//  Created by Elias Abel on 4/12/17.
//
...
extension URLSessionConfiguration {
    ...
    public class Builder {
        ...
        public enum LeafMultipathServiceType: Int {
        ...
        public private(set) var multipathServiceType: LeafMultipathServiceType?

        public init(_ configuration: URLSessionConfiguration? = nil) {

            cache = LeafRequest.LeafCachePolicy(rawValue: configuration?.requestCachePolicy.rawValue ?? 0)

            requestTimeout = configuration?.timeoutIntervalForRequest

            resourceTimeout = configuration?.timeoutIntervalForResource

            serviceType = LeafRequest.LeafServiceType(rawValue: configuration?.networkServiceType.rawValue ?? 0)
            ...
            protocolClasses = configuration?.protocolClasses
                ...
                if #available(iOS 11.0, *) {
                    self.multipathServiceType = LeafMultipathServiceType(rawValue: configuration?.multipathServiceType.rawValue ?? 0)
                }
            #endif
        }

        @discardableResult open func setCache(_ cachePolicy: LeafRequest.LeafCachePolicy?) -> Self {
            self.cache = cache
            return self
        }
        ...
        @discardableResult open func setMultipathServiceType(_ multipathServiceType: LeafMultipathServiceType?) -> Self {
