...
//    }
    ...
    @discardableResult
    public func async(_ method: LeafRequest.LeafMethod,
    ...
    @discardableResult
    public func sync(_ method: LeafRequest.LeafMethod, timeout: TimeInterval = 120) throws -> (leaf: Leaf, response: LeafResponse) {
...
//    public func cached() throws -> LeafResponse {
//        let request = try self.build(method: method, timeout: timeout)
//        let task = self.session.dataTask(request)
//        return try task.cached()
//    }
}

internal func dumping(_ item: Any?) -> String {
    var output = ""
